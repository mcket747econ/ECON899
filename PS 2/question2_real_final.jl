...
function distribution(prim::Primitives, res::Results, tol::Float64 =-1e3) ##Function to calculate the wealth distribution
    ...
    for a_index=1:na,s_index=1:ne #We iterate over assets and over states
        ...
        #1. For a given asset_prime value we then want to obtain the index of the  A array value that matches ap
        #2, To Accomplish this we take the difference of our asset_prime value and all possible asset values from A that household can choose.
        #3 We then find the index of of the  Asset value equal to the ap given
        #4 So we want the magnitude of the difference to be 0
