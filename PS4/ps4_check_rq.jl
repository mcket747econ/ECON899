#####Status: Preliminary at the moment.
#Problem Set 4 Code
...
function Initialize(theta_val::Float64)
    prim = Primitives(theta=theta_val)
...
prim, res = Initialize(0.11)
# print(prim)
pop_weights,pop_sum,pop_normal, F =  Fdist_sum(prim,res)
@time res11 = Run_all(prim,res, 1e-1, .12)

prim, res = Initialize(0.0)
@time res0 = Run_all(prim,res, 1e-1, .12)


mutable struct change_primitives
    TΔ::Float64
    T::Float64
    K_t::Array{Float64,T}
    L_t::Array{Float64,T}
...
function update(T::Float64=30,steps::Float64=30)
    TΔ::Float64
    K_t::Array{Float64,T}
    L_t::Array{Float64,T}
    TΔ::Float64 = (res11.K-res0.K)/prim.T     ##equally spaced
    for t in T
        K_t[t] = K_0 + prim.delta*t/change_prims.N
    end
    change_primitives(TΔ,T,K_t,L_t)
end

###Rough - adjust
function ShootBackward(tol::Float64 = 1e-3)
    ##Update 
    for t = T-1:-1:1 
        BackwardInduct(prim::Primitives,res::Results)
    end
end

function BackwardInduct(jN::float64,tol::Float64 = 1e-3)
    ##Update 
    for j = jn:-1:1
        BackInduct(prim::Primitives,res::Results)
    end
end


function ShootForward(tol::Float64 = 1e-3)
    ##Update 
    res
end