...
@with_kw struct Grids
    k_lb::Float64 = 0.001
    k_ub::Float64 = 15.0 #changed from 20 to 15 following PS5
    n_k::Int64 = 21
    k_grid::Array{Float64,1} = range(k_lb, stop = k_ub, length = n_k)

    K_lb::Float64 = 11.0 #changed from 10 to 11 following PS5
    K_ub::Float64 = 15.0
    n_K::Int64 = 11
    K_grid::Array{Float64,1} = range(K_lb, stop = K_ub, length = n_K)
...
function DrawShocks(S::Shocks, N::Int64,T::Int64, sd::Int64=12032020)
    ...
    # Shock
    Random.seed!(sd) #Jacob Edit: Make seed a variable, shouldn't be defined in equation like this
    dist = Uniform(0, 1)
