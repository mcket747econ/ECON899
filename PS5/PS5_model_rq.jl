...
function initialize_mut(G::Grids) #See PS5 for choices, this could be rewritten as an overall initilize or put into a bigger initilzer 
    # @unpack a0, b0, a1, b1, R2 = R ## Trying without 
    ...
    pf_k = zeros(Float64,n_k, n_eps, n_K,n_z)
    pf_v = zeros(Float64,n_k, n_eps, n_K,n_z)
    return a0, b0, a1, b1, R2, pf_k, pf_v 
end

function SimulateCapitalPath(R::Results, P::Params, Epsilon::Array{Float64, 2},Z::Array{Float64, 1}) #need to figure out the proper bounds on this, right now I think its to T-1
    @unpack pf_k,pf_v = R
    @unpack N, T, burn = P
    @unpack K_grid = G
